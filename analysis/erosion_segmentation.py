...
for filename in tqdm(files[:10]):
    ...
    eroded_mask = ErosionSegmentation().segment(mask)
    eroded_labels = measure.label(eroded_mask, connectivity=1)
    eroded_props = measure.regionprops_table(eroded_labels, properties=props_list)
    eroded_props['complexity'] = (eroded_props['perimeter_crofton']
    ...
    axs[0].imshow(labeled > 0, cmap='viridis')
    axs[0].set_title('Original')
    axs[1].imshow(eroded_mask > 0, cmap='viridis')
