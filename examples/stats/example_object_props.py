...
plt.figure()
plt.imshow(heart_slice.total_segments, cmap='viridis')
plt.show()
...
print(objects_props.info())

objects_props = objects_props_builder.objects_props.loc[:, ['segment_labels',
...
relative_orientation = objects_props.groupby('location',
                                             observed=True)['relative_orientation'].apply(list)

print(type(relative_orientation))

fig, axs = plt.subplots(ncols=3)
for i, orientation in enumerate(relative_orientation):
    axs[i].hist(orientation, bins=20)
plt.show()
